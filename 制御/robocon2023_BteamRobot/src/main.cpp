...
void robot_carry(){
	...
	x_vol = (float)ps3_data.LxPad/63.0*3500.0;
	y_vol = (float)ps3_data.LyPad/63.0*3500.0;
...
void LEDtape_func(int red,int blue,int green){
	LEDtape[0].write(red?HIGH:LOW);
	LEDtape[1].write(blue?HIGH:LOW);
	LEDtape[2].write(green?HIGH:LOW);
}

void LED_select(){
	static bool past_sw = false,LED_flag;
	if(!Bsw.read() && !past_sw) LED_flag  = !LED_flag;
	if(LED_flag){
		LEDtape_func(100,0,0);
	}else{
		LEDtape_func(0,100,0);
	}
	past_sw = !Bsw.read();
}
