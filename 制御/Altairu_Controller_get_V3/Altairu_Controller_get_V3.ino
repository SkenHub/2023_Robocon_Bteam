...
void updateSpeeds() {
  //ウォーム速度
  if(data[0] == "1" && data[2] == "0" && limitSwitches[0]){
    speed1 =  255 * spk1;
  }else if(data[0] == "0" && data[2] == "1" && limitSwitches[1] && !limitSwitches[3]){
    speed1 = -255 * spk1;
  }else{ speed1 = 0; }
  //ロジャー速度
  if(data[1] == "1" && data[3] == "0" && limitSwitches[2] && !limitSwitches[0]){
    speed2 = -255 * spk2;
  }else if(data[1] == "0" && data[3] == "1" && limitSwitches[3]){
    speed2 =  255 * spk2*0.2;
  }else{ speed2 = 0; }
  //お助け速度
  if(data[4] == "1" && data[6] == "0" && limitSwitches[4]){
    speed3 = -255 * spk2;
